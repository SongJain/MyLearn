...
public class SelectViewadress {                              
	...
	class Search implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			try {
				ResultSet rs = new AdminDao().selectstuaddress(searchtext);
				...
				while(rs.next())
				{
					Vector<String> v = new Vector<>();   //在创建一个一维vector 读取指定列元素
					for(int i = 0 ;i<rd.getColumnCount();i++)
					{
						v.add(rs.getString(i+1));
					}
					data.add(v);         //添加指定元素进二维向量
				}				
